...
if (!isset($_SESSION["nombre"])) {
	...
	if ($_SESSION['rPagos'] == 1) {
		require_once "../modelos/Reporte_bancos.php";

		$tickets = new ReporteBancos();
		...
		switch ($_GET["op"]) {
			...
			case 'listar':
				...
				if ($cargo == "superadmin" || $cargo == "admin") {
					if ($fecha_inicio == "" && $fecha_fin == ""  && $banco == "") {
						$rspta = $tickets->listar();
					} elseif ($fecha_inicio != "" && $fecha_fin != "" && $banco != "") {
						$rspta = $tickets->listarPorFechaYbanco($banco, $fecha_inicio, $fecha_fin);
					} elseif ($fecha_inicio != "" && $fecha_fin != "" && $banco == "") {
						$rspta = $tickets->listarPorFecha($fecha_inicio, $fecha_fin);
					} else {
						$rspta = $tickets->listarPorBanco($banco);
					}
				} elseif ($cargo == "vendedor_total") {
					if ($fecha_inicio == "" && $fecha_fin == ""  && $banco == "") {
						$rspta = $tickets->listarPorUsuario($idusuario);
					} elseif ($fecha_inicio != "" && $fecha_fin != "" && $banco != "") {
						$rspta = $tickets->listarPorFechaYbancoUsuario($idusuario, $banco, $fecha_inicio, $fecha_fin);
					} elseif ($fecha_inicio != "" && $fecha_fin != "" && $banco == "") {
						$rspta = $tickets->listarPorUsuarioFecha($idusuario, $fecha_inicio, $fecha_fin);
					} else {
						$rspta = $tickets->listarPorBancoUsuario($idusuario, $banco);
					}
				}
